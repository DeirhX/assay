...
| Risk lens | `tools/risk.py` | Correlation, volatility, effective-bets, and factor-shock stress over held names, plus an `fx` block: non-base currency exposure and the window's FX move / estimated CZK contribution (`GET /api/risk`). |
| FX panel | `tools/fx_history.py`, `data/cache/fx-history.json` (gitignored) | Incremental daily FX-rate panel (`USDCZK`/`EURCZK` → base per 1 unit of foreign) from Yahoo `<CCY><BASE>=X` pairs — the FX-clean spine for currency attribution (`update_panel`/`load_panel`/`pair_series`/`rate_on`). Topped up alongside the IBKR history sync (`holdings_sync`). The currency lens (`exposure_by_currency`/`window_report`) feeds the Risk view's `fx` block. |
| Process attribution | `tools/attribution.py` | Did the process earn its keep? Actual time-weighted return over the window vs never-rebalanced (freeze the book at the start, let prices run) and benchmark (same koruna into SPY/QQQ) counterfactuals — deposits neutralized, foreign prices converted day-by-day through the FX panel (`GET /api/attribution`, Portfolio → Attribution). Pure engine (`time_weighted_return`/`positions_at`/`hold_index`/`flow_curve`); read-only, degrades with caveats. A compact headline verdict is cached (`cache_verdict`/`load_verdict`, `data/cache/attribution-verdict.json`) when the view is opened and surfaced on the **Today** cockpit — no network on the Today path. |
...
| Project skills | `.claude/skills/` | Shared agent instructions for future chats. |
...
See `.claude/skills/perplexity-deep-research/SKILL.md` for the exact browser
automation workflow.
...
assets (`site.css`), `.claude/skills/`, and docs. The derived holdings summary
