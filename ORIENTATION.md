...
| Process attribution | `tools/attribution.py` | Did the process earn its keep? Actual time-weighted return over the window vs never-rebalanced (freeze the book at the start, let prices run) and benchmark (same koruna into SPY/QQQ) counterfactuals — deposits neutralized, foreign prices converted day-by-day through the FX panel (`GET /api/attribution`, Portfolio → Attribution). Pure engine (`time_weighted_return`/`positions_at`/`hold_index`/`flow_curve`); read-only, degrades with caveats. |
